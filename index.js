...
const node_dependencies = new TreeNode("file", "node_dependencies.js", "08-15-2022", "8kb");
...
const src = new TreeNode("folder", "Src", "08-16-2022", "4kb", components, assets);
const publicFolder = new TreeNode("folder", "PublicFolder", "08-16-2022", "4kb", index);
const node_modules_folder = new TreeNode("folder", "node_modules", "08-16-2022", "4kb", node_dependencies, yaml, babel);
const rootFolder = new TreeNode("folder", "Main", "08-17-2022", "3Mb", publicFolder);
...
const renderRightSidePane = (resultSearchTree) => {
    ...
    for (let i = 0; i < resultSearchTree.children?.length; i++) {
        ...
        Object.keys(resultSearchTree.children[i]).map((key) => {
            const iconFolder = resultSearchTree.children[i].type === 'folder'
            if (interestedField.has(key)) {
                if (key === 'name') {
                    if (resultSearchTree.children[i].type === 'folder') {
                        tableDupRow += '<td class="rowName">' + '<img class="folderSVG" src="./Icons/Folder.svg" />' + resultSearchTree.children[i].name + '</td>'
                    } else {
                        tableDupRow += '<td class="rowName">' + '<img class="fileSVG" src="./Icons/File.svg" />' + resultSearchTree.children[i].name + '</td>'
                    }
                }
                if (key === 'modified') {
                    tableDupRow += '<td>' + resultSearchTree.children[i].modified + '</td>'
                }
                if (key === 'size') {
